    ...
    C: Has<XSpanIdString> + Has<Option<AuthData>> + Clone + Send + Sync + 'static,
{
    ...
    async fn add_student(
        &self,
        param_student_upsert: models::StudentUpsert,
        context: &C) -> Result<AddStudentResponse, ApiError>
    {
        ...
        // Body parameter
        let body = serde_json::to_string(&param_student_upsert).expect("impossible to fail to serialize");
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<models::StudentUpsert>(body)?;
                Ok(AddStudentResponse::SuccessfulOperation
                    (body)
                )
            }
            405 => {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::StudentUpsert>(body)?;
                Ok(UpdateStudentResponse::SuccessfulOperation
                    (body)
                )
            }
            400 => {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::StudentUpsert>(body)?;
                Ok(GetStudentByIdResponse::SuccessfulOperation
                    (body)
                )
            }
            400 => {
    ...
    async fn get_students(
        ...
        param_undergraduate_school: Option<String>,
        param_date_of_birth: Option<chrono::DateTime::<chrono::Utc>>,
        param_place_of_birth: Option<String>,
        param_polity_name: Option<String>,
        param_specialism: Option<String>,
        param_sorts: Option<&Vec<models::StudentSortCriteria>>,
        ...
        context: &C) -> Result<GetStudentsResponse, ApiError>
    {
        ...
        let query_string = {
            ...
            if let Some(param_phone) = param_phone {
                query_string.append_pair("phone",
                    &param_phone.to_string());
            }
            if let Some(param_undergraduate_school) = param_undergraduate_school {
                query_string.append_pair("undergraduateSchool",
                    &param_undergraduate_school.to_string());
            }
            if let Some(param_date_of_birth) = param_date_of_birth {
                query_string.append_pair("dateOfBirth",
                    &param_date_of_birth.to_string());
            }
            if let Some(param_place_of_birth) = param_place_of_birth {
                query_string.append_pair("placeOfBirth",
                    &param_place_of_birth.to_string());
            }
            if let Some(param_polity_name) = param_polity_name {
                query_string.append_pair("polityName",
                    &param_polity_name.to_string());
            }
            if let Some(param_specialism) = param_specialism {
                query_string.append_pair("specialism",
                    &param_specialism.to_string());
            }
            if let Some(param_sorts) = param_sorts {
                query_string.append_pair("sorts",
                    &param_sorts.iter().map(ToString::to_string).collect::<Vec<String>>().join(","));
            }
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::StudentViewCollection>(body)?;
                Ok(GetStudentsResponse::SuccessfulOperation
                    (body)
                )
            }
            400 => {
